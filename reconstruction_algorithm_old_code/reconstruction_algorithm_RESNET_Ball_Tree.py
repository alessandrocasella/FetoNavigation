...
# It creates a Ball Tree useful for the search of the key frame which is the closest to the frame that must be relocalized
...
# This function extracts the features from a single image and returns a flatten numpy array with these features
...
def sanity_check(name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha, ill_beta, rot, crop_red, patch_dim):
    ...
    len_features_image_to_check = len(features_image_to_check)
    features_image_to_check = np.reshape(features_image_to_check, (1, len_features_image_to_check))

    features_frames_tree = np.concatenate((features_image_to_check, features_key_frames), axis=0)
    features_to_query = np.reshape(features_frames_tree[0,:], (1, len_features_image_to_check))
    leafSize = 4
    tree = indexBallTree(features_frames_tree, leafSize)
    dist, ind = tree.query(features_to_query, k=2, breadth_first=True)
    #print(ind[-1][-1])
...
name_experiment = 'ResNet18_Ball_Tree_BF_4' #string containing the name used to identify the experiment
