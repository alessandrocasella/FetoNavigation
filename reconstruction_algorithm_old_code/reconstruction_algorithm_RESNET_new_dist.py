...
def sanity_check(name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha, ill_beta, rot, crop_red, patch_dim):
    ...
    for i in range(features_key_frames.shape[0]):
        ...
        features_image_to_check_tensor = torch.from_numpy(features_key_frames[i, :]) #.unsqueeze(dim=0) #.unsqueeze(dim=0)
        features_key_frames_vector_tensor = torch.from_numpy(features_image_to_check) #.unsqueeze(dim=0)
        dist = torch.dot(features_key_frames_vector_tensor / torch.norm(features_image_to_check_tensor),features_key_frames_vector_tensor / torch.norm(features_key_frames_vector_tensor)).cpu().numpy()
...
name_experiment = 'ResNet50_new_Dist' #string containing the name used to identify the experiment
