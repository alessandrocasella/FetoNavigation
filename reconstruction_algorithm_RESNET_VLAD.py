...
name_experiment = 'ResNetVLAD_FINAL_ALL_VIDEOS'   #string containing the name used to identify the experiment
...
# Class of the used network, derived from RESNET structure
...
def VLAD(descriptor_array, visual_dictionary):
    #print('dentro VLAD', descriptor_array.shape)
...
def extract_feature_image(img):
    ...
    # using RESNET and appended in the list features
    ...
    #print('in extract features image', features.shape)
...
def compute_features(name_experiment, name_video, list_images, path_images, num_features_image):
    ...
    path_visual_dictionary = os.path.join(DATASET_FILES_PATH, 'visual_dictionary', name_experiment)
    if not os.path.exists(path_visual_dictionary):
        os.makedirs(path_visual_dictionary)
    ...
    if not os.path.exists(path_dictionary_npy):
        list_array_features = []  # list of arrays containing the features of the images in list_images
        ...
        for i in range(features_array.shape[0]):
            ...
            #features = features.flatten()
            #print('shape of features in compute features',features.shape)
            list_array_features.append(features)
            if i == 0:
                all_features_list = np.copy(features) # array containing the features of all the elements in list_images
            else:
                all_features_list = np.concatenate((all_features_list, features), axis=0)

        # computation of VLAD visual dictionary
        #print('shape of all features list', all_features_list.shape)
        visual_dictionary = compute_visual_dictionary(all_features_list)

        for i in range(len(list_array_features)):
            vlad_vector = VLAD(list_array_features[i], visual_dictionary)
            vlad_vector = np.array(vlad_vector)
            features_array[i, :] = vlad_vector

        np.save(path_dictionary_npy, features_array)   # saving features in a numpy file

        file = path_visual_dictionary +'/'+ name_video+".pickle"
        with open(file, 'wb') as f:
            pickle.dump(visual_dictionary, f)   # saving visual dictionary

    else:
        features_array = np.load(path_dictionary_npy)
        file_path = path_visual_dictionary+'/'+name_video+'.pickle'
        visual_dictionary = pd.read_pickle(file_path)

    return features_array, visual_dictionary
...
def sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha, ill_beta, rot, crop_red, patch_dim):
    ...
    features_image_to_check = VLAD(features_image_to_check, visual_dictionary)
    features_image_to_check = np.array(features_image_to_check )
    #features_image_to_check = features_image_to_check.flatten()

    list_differences = []
    #print('shape of features key frame in sanity check', features_key_frames.shape)
...
for name_video in list_videos:
    ...
    #num_features_image = 65536   # for RESNET18 and RESNET34 with k=128
    #num_features_image = 32768   # for RESNET18 and RESNET34 with k=64
    #num_features_image = 16384   # for RESNET18 and RESNET34 with k=32
    #num_features_image = 8192   # for RESNET18 and RESNET34 with k=16

    #num_features_image = 262144  # for RESNET50 with k=128
    #num_features_image = 131072  # for RESNET50 with k=64
    num_features_image = 65536  # for RESNET50 with k=32
    #num_features_image = 32768  # for RESNET50 with k=16
    ...
    features_key_frames, visual_dictionary = compute_features(name_experiment, name_video, list_key_frames, path_images, num_features_image)


    list_metric_ssim = []
    # sanity check with no transformation applied
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=0, crop_red=0, patch_dim=0)
    experiment_result[0, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with 10 degrees rotation
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=10, crop_red=0, patch_dim=0)
    experiment_result[1, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with 30 degrees rotation
    tuple_result = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=30, crop_red=0, patch_dim=0)
    experiment_result[2, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)


    # sanity check with 60 degrees rotation
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=60, crop_red=0, patch_dim=0)
    experiment_result[3, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast alpha=0.8
    tuple_result = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=0.80, ill_beta=1.0, rot=0, crop_red=0, patch_dim=0)
    experiment_result[4, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast alpha=0.9
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=0.90, ill_beta=1.0, rot=0, crop_red=0, patch_dim=0)
    experiment_result[5, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast alpha=1.10
    tuple_result = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.10, ill_beta=1.0, rot=0, crop_red=0, patch_dim=0)
    experiment_result[6, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast alpha=1.20
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.20, ill_beta=1.0, rot=0, crop_red=0, patch_dim=0)
    experiment_result[7, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast beta=0.8
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames,list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=0.8, rot=0, crop_red=0, patch_dim=0)
    experiment_result[8, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast beta=0.9
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames,list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=0.9, rot=0, crop_red=0, patch_dim=0)
    experiment_result[9, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast beta=1.10
    tuple_result = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames,list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.10, rot=0, crop_red=0, patch_dim=0)
    experiment_result[10, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with contrast beta=1.20
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames,list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.20, rot=0, crop_red=0, patch_dim=0)
    experiment_result[11, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check crop reduction=20
    tuple_result = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=0, crop_red=20, patch_dim=0)
    experiment_result[12, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check crop reduction=30
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=0, crop_red=30, patch_dim=0)
    experiment_result[13, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check crop reduction=50
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=0, crop_red=50, patch_dim=0)
    experiment_result[14, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    # sanity check with patch insertion of patch_dim=100
    tuple_result, metric_ssim = sanity_check(visual_dictionary, name_experiment, frame_to_reattach, matrix_frame_to_reattach, panorama, features_key_frames, list_key_frames, matrices_key_frames, path_images, mask, ill_alpha=1.0, ill_beta=1.0, rot=0, crop_red=0, patch_dim=100)
    experiment_result[15, index_column] = tuple_result
    list_metric_ssim.append(metric_ssim)

    np_metric_ssim = np.array(list_metric_ssim)
    path_folder = os.path.join(DATASET_FILES_PATH, 'boxplots_npy', name_experiment)
    if not os.path.exists(path_folder):
        os.makedirs(path_folder)
