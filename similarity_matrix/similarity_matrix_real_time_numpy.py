...
# It calculates the SSIM between the input images
...
# It regulates the entire process of creation of the real time matrix. It creates a video where on the left there is
# the original sequence, while on the rigth the similarity matrix where the given frame is exactly in the centre
def similarity_matrix_rt(frames_list, path, buffer_windows, shape, path_final_video):
    print((shape*2, shape))
    ...
    fps = 25.0
    video_matrix = cv2.VideoWriter(path_final_video, cv2.VideoWriter_fourcc(*'mp4v'), fps, (shape*2, shape))
    ...
    for count in range(num_frame_buffer,len(frames_list)-num_frame_buffer, 1):
        ...
        min = np.min(current_map_to_plot)
        current_map_to_plot_scaled = np.array([(x - min) / (max - min) for x in current_map_to_plot])
        img = cv2.imread(path + '/' + frames_list[num_frame_buffer+count], 1)
        img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        np_horiz_concat = np.concatenate((img, current_map_to_plot_scaled * 255), axis=1)
        np_horiz_concat = np.uint8(np_horiz_concat)
        np_horiz_concat = cv2.cvtColor(np_horiz_concat, cv2.COLOR_GRAY2RGB)
...
#         # print(os.path.join(os.getcwd(), 'similarity_matrices_rt', name_video_list[count] + '.mp4'))
#         if not os.path.exists(os.path.join(os.getcwd(), 'similarity_matrices/similarity_matrices_rt/', name_video_list[count] + '.mp4')):
...
# MAIN FLOW EXECUTION WITH DATASET FROM MICCAI 2020
...
if __name__ == '__main__':
    ...
    for count in range(len(name_video_list)):
        path_final_video = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/similarity_matrices_rt_gb_9', name_video_list[count] + '.mp4')
        if not os.path.exists(path_final_video):
            ...
            shape = img.shape[0]
            buffer_windows = 50
            similarity_matrix_rt(frames_list, path, buffer_windows, shape, path_final_video)