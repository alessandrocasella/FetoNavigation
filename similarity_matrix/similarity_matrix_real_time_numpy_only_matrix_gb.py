...
def calc_SSIM(im1_gray, im2_gray):
    im1_gray_gb = cv2.GaussianBlur(im1_gray, (9, 9), cv2.BORDER_DEFAULT)
    im2_gray_gb = cv2.GaussianBlur(im2_gray, (9, 9), cv2.BORDER_DEFAULT)
...
# It regulates the entire process of creation of the real time matrix. It creates a video where
# we can see the similarity matrices of all the videos in buffer windows of 50 frames
# The matrix is not scaled and has the original size
def similarity_matrix_rt(frames_list, path, buffer_windows, shape, name_video):
    ...
    video_path = os.path.join(os.getcwd(),
                              'dataset_MICCAI_2021_files/similarity_matrices/similarity_matrices_rt_only_matrix_gb_9', name_video)
    video_matrix = cv2.VideoWriter(video_path + '.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (buffer_windows, buffer_windows))

    current_map_to_plot = sim_matrix_buffer*255
    current_map_to_plot = np.uint8(current_map_to_plot)
    current_map_to_plot = cv2.cvtColor(current_map_to_plot, cv2.COLOR_GRAY2RGB)
    video_matrix.write(current_map_to_plot)
    ...
    for count in range(num_frame_buffer,len(frames_list)-num_frame_buffer, 1):
        ...
        current_map_to_plot = current_map*255
        current_map_to_plot = np.uint8(current_map_to_plot)
        current_map_to_plot = cv2.cvtColor(current_map_to_plot, cv2.COLOR_GRAY2RGB)
        video_matrix.write(current_map_to_plot)
...
if __name__ == '__main__':
    ...
    for count in range(len(name_video_list)):
        if not os.path.exists(os.path.join(os.getcwd(),
                                           'dataset_MICCAI_2021_files/similarity_matrices/similarity_matrices_rt_only_matrix_gb_9/', name_video_list[count] + '.mp4')):
            ...
            path_image = os.path.join(os.getcwd(), path_video_list[count], frames_list[0])
