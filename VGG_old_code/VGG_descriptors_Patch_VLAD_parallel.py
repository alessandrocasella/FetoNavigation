...
# Model class derived from VGG 16 structure
...
def compute_features_patch(patches, i, j):

    # For every patch features are computed using VGG16 and appended in the list features
    print(str(i), str(j))
    img = patches[i, j, :, :, :]
...
# This function extracts the features from a single image, divides it in patches, uses multiprocessing
# to compute the array of total features and then applies VLAD to reduce the array to a vector
def extract_feature_image(img, length_list_images):


    # Division of the image in patches of 64x64 and stride = 58.
    patches = patchify(img, (64, 64, 3), step=58)
    #print('shape of patches: ', patches.shape)

    # This array will contain the features of all the patches from an image
    features = np.zeros((patches.shape[0]*patches.shape[1], 4096), dtype=float)


    ctx = multiprocessing.get_context("spawn")
    pool = ctx.Pool(processes=4)

    # Creation of the ranges for the index i and j

    # i is equal to 0,0,0...1,1,1...2,2,2...
    lst_i = range(patches.shape[0])
    range_i = list(itertools.chain.from_iterable(itertools.repeat(x, patches.shape[0]) for x in lst_i))

    lst_j = range(patches.shape[1])
    range_j = islice(cycle(lst_j), len(lst_j) * patches.shape[0])

    pool_result = pool.starmap(compute_features_patch, zip(repeat(patches), range_i, range_j))
    pool.close()
    pool.join()

    for line, feature in enumerate(pool_result):
        features[line, :] = feature.cpu().detach().numpy().reshape(-1)
...
def compute_features_original(features_array, name_video):
    path_dictionary_npy = os.path.join(os.getcwd(), 'dataset_MICCAI_2020_files/dictionary_file_npy/Patch+VLAD_parallel', 'dictionary_original_patches_' + name_video + '.npy')
    if not os.path.exists(path_dictionary_npy):
        for i in range(features_array.shape[0]):
            print(i)
            img = cv2.imread(path_folder_frames + '/' + list_frames[i])
            features = extract_feature_image(img, features_array.shape[0])
...
if __name__ == '__main__':
    ...
    features_array = np.zeros((len(list_frames), 20480), dtype=float)

    #these functions perform the feature extraction and the sanity check with the original images
    features_array = compute_features_original(features_array, name_video)
