...
# It applies an improved version of VLAD to the descriptor array in input given a visual dictionary
...
def extract_feature_image(img):
    ...
    features = []
    print(img.shape)
    patches = patchify(img, (64, 64, 3), step=16)
    print('shape of patches: ', patches.shape)
    #features = np.zeros((patches.shape[0], patches.shape[1]), dtype=float)

    # Division of the image in patches of 64x64 and stride = 16. for every patch features are computed
    ...
    vlad_vector = improvedVLAD(features, visual_dictionary)
...
def compute_features_original(features_array, name_video):
    path_dictionary_npy = os.path.join(os.getcwd(), 'dataset_MICCAI_2020_files/dictionary_file_npy/Patch+VLAD', 'dictionary_original_patches_' + name_video + '.npy')
