...
# It computes the features of a patch (given the correspondent index) using weigths of VGG16
def compute_features_patch(batch_array, index):
    print(index)
    img = batch_array[index, :, :, :, :]
...
# It launches the feature extraction calculation using multiprocessing
def extract_feature_batch(batch_array, features):
    index = range(batch_array.shape[0])

    ctx = multiprocessing.get_context("spawn")
    pool = ctx.Pool(processes=4)

    pool_result = pool.starmap(compute_features_patch, zip(repeat(batch_array), index))
    pool.close()
    pool.join()
...
def extract_feature_image(img):
    ...
    for i in range(0, patches.shape[0], batchsize):
        ...
        #print(X_batch.shape)
        features = extract_feature_batch(X_batch, features)

    # Convert to NumPy Array
    features = np.array(features)
    print('features after casting to array', features.shape)
    #apply VLAD method
