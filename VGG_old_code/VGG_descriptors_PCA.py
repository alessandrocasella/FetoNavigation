...
def compute_features_original(list_frames, features_array_buffer, name_video):
    path_dictionary_npy = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/dictionary_file_npy', 'dictionary_original_all_video_' + name_video + '.npy')
    ...
    features_array_buffer = pca.fit_transform(features_array_buffer)
    print(features_array_buffer.shape)
...
# It computes feature extraction for all images in the buffer and creates an external npy file with the features array
def compute_features_transformation(list_frames, features_array_buffer, name_video):
    ...
    features_array_to_check = np.copy(features_array_buffer)
    ...
    return features_array_buffer, features_array_to_check



# It performs a sanity check on transformed images: applies L2 between the given frames and all frames and computes
# ssim between the given frames and all the others
def sanity_check_transformation(list_frames, features_array_buffer, path_folder_frames, features_array_to_check):
...
features_array_buffer = np.zeros((length_list,4096), dtype=float)

# These functions perform the feature extraction and the sanity check with the original images
#features_array_buffer = compute_features_original(list_frames, features_array_buffer, name_video)
...
features_array_buffer, features_array_to_check = compute_features_transformation(list_frames, features_array_buffer, name_video)
sanity_check_transformation(list_frames, features_array_buffer, path_folder_frames, features_array_to_check)
