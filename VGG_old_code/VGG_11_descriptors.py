...
def compute_features_original(features_array_buffer, name_video):
    path_dictionary_npy = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/dictionary_file_npy/VGG11', 'dictionary_original_' + name_video + '.npy')
...
class FeatureExtractor(nn.Module):
    def __init__(self, model):
        ...
        # Extract VGG-11 Feature Layers
        self.features = list(model.features)
        self.features = nn.Sequential(*self.features)
        # Extract VGG-11 Average Pooling Layer
        self.pooling = model.avgpool
        # Convert the image into one-dimensional vector
        self.flatten = nn.Flatten()
        # Extract the first part of fully-connected layer from VGG11
