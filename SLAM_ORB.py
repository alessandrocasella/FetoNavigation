...
name_experiment = 'SLAM_ORB'  # string containing the name used to identify the experiment
...
def ORB_pair(image_pair, mask, list_homographies, list_key_frames, path_frames, name_frame_to_relocalize, tracking_lost):
    ...
    kp_0, desc_0 = orb.detectAndCompute(img0_raw, None)
    kp_1, desc_1 = orb.detectAndCompute(img1_raw, None)
    ...
    try:
        ...
        if (h_matrix is None):
            ...
            kp_kf, desc_kf = orb.detectAndCompute(kf_min_image, None)
            kp_0, desc_0 = orb.detectAndCompute(img0_raw, None)
...
def ORB_pair_relocalize(image_pair, mask):
    img0_raw = image_pair[0]
    img1_raw = image_pair[1]
...
def compute_key_frames_mosaic(list_frames, mask, name_video):
    ...
    for i in t:
        ...
        list_homographies, kp_0, desc_0, kp_1, desc_1, tracking_lost = ORB_pair(image_pair, mask, list_homographies, list_key_frames, os.path.join(DATASET_PATH, name_video, 'images'), list_frames[i], tracking_lost)
...
def relocalize(frame_to_relocalize_transf, mask, list_key_frames, list_key_frames_names, name_frame_to_relocalize):
    ...
    for key_frame in list_key_frames:
        ...
        src_pts, desc_0, dst_pts, desc_1 = ORB_pair_relocalize(image_pair, mask)
...
def sanity_check(name_frame_to_relocalize, frame_to_relocalize_transf, path_frames, panorama, mask, matrix_frame_to_relocalize,
    ...
    # METRIC CALCULATION
    metric_ssim = compute_metric(name_frame_to_relocalize,tuple_result[2], tuple_result[3], path_frames, is_relocalization=True)
    print(metric_ssim)
