...
def panorama_initialization(name_video, mask, name_experiment, first_frame_name):
    path_panorama_image_folder = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/output_panorama_images',
                                              name_experiment)
    ...
    path_panorama_video_folder = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/output_panorama_video',
                                              name_experiment)
    ...
    path_panorama_image = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/output_panorama_images', name_experiment,
    ...
    if not os.path.exists(path_panorama_image):
        ...
        # Panorama initialization --> first image in the centre
        initial_img_pth = os.path.join(os.getcwd(), '../dataset_MICCAI_2020/dataset', name_video, 'images',
                                       first_frame_name)
        initial_img = cv2.imread(initial_img_pth, 1)
        initial_img = cv2.resize(initial_img, (NEW_SHAPE, NEW_SHAPE))
        mask = cv2.cvtColor(mask, cv2.COLOR_GRAY2RGB)
        image_circle = cv2.bitwise_and(initial_img, mask)
        erosion_size = 10
        erosion_shape = cv2.MORPH_ELLIPSE
        element = cv2.getStructuringElement(erosion_shape, (2 * erosion_size + 1, 2 * erosion_size + 1),
                                            (erosion_size, erosion_size))
        ...
        img_origin = np.array(
            [[np.cos(0), np.sin(0), -image_circle.shape[1] / 2], [-np.sin(0), np.cos(0), -image_circle.shape[0] / 2],
             [0, 0, 1]])
        H = img_origin @ initial_matrix
        panorama_curr = cv2.warpPerspective(image_circle, np.float32(H), (panorama.shape[1], panorama.shape[0]),
                                            flags=cv2.INTER_NEAREST)
...
def panorama_reconstruction_rt(panorama, img1_pth, H, new_H, out, mask):
    image = cv2.imread(img1_pth, 1)
    image = cv2.resize(image, (NEW_SHAPE, NEW_SHAPE))
    image_circle = cv2.bitwise_and(image, mask)
    erosion_size = 10
    erosion_shape = cv2.MORPH_ELLIPSE
...
def nn_match_two_way(desc1, desc2, nn_thresh):
    ...
    # # print(desc1.shape)
    # print('descriptors')
    # print(np.unique(desc1))
    # print(np.unique(desc2))
    # print(desc2.shape)
    # print(desc2)
    ...
    #print(dmat.shape[0])
    #print(np.unique(idx))
    #print(np.arange(dmat.shape[0]))
    scores = dmat[np.arange(dmat.shape[0]), idx]
    #print(np.unique(scores)) # SONO TUTTI ZERO

    # Threshold the NN matches.
    keep = scores < nn_thresh
    #print(keep)
    #print(np.unique(keep))
    ...
    while SEQ_IS_END == False:
        ...
        try:
            ...
            if (i == 0):
                ...
                tuple_key_frame = (actual_key_frame, H_key_frame, feature_space0.cpu().detach().numpy(),
                                   descriptors_img0.cpu().detach().numpy())
                ...
                if (actual_matches_points.shape[0] < NUM_POINTS_THRESHOLD):
                    print('HO SUPERATO LA THRESHOLD')
                    list_matches = []
                    for elem in list_key_frames:
                        print(descriptors_img0.shape)
                        #pydevd_pycharm.settrace('10.79.251.35', port=8200, stdoutToServer=True, stderrToServer=True)
                        matches = nn_match_two_way(np.transpose(descriptors_img0.cpu().detach().numpy()), np.transpose(elem[3]), 0.5)
                        print('candidato_key_frame',list_frames[i])
                        print('current_key_frame',elem[0])
                        list_matches.append(matches.shape)
...
def compute_key_frames_mosaic(list_frames, mask, matcher, name_video):
    ...
    with Manager() as manager:
        ...
        for i in t:
            ...
            img1_pth = os.path.join(os.getcwd(), '../dataset_MICCAI_2020/dataset', name_video, 'images',
                                    list_frames[i + 1])
...
def compute_metric(img1, img2, mat1, mat2, h_estim_1_2, path_frames, mask):
    ...
    img1_warped = cv2.warpPerspective(img1, np.float32(mat1),
                                      (panorama_chain_rule.shape[1], panorama_chain_rule.shape[0]),
                                      flags=cv2.INTER_NEAREST)
    mask_img1 = cv2.warpPerspective(mask, np.float32(mat1),
                                    (panorama_chain_rule.shape[1], panorama_chain_rule.shape[0]),
                                    flags=cv2.INTER_NEAREST)
    np.copyto(panorama_chain_rule, img1_warped, where=mask_img1.astype(bool))
    img2_warped = cv2.warpPerspective(img2, np.float32(mat2),
                                      (panorama_chain_rule.shape[1], panorama_chain_rule.shape[0]),
                                      flags=cv2.INTER_NEAREST)
    mask_img2 = cv2.warpPerspective(mask, np.float32(mat2),
                                    (panorama_chain_rule.shape[1], panorama_chain_rule.shape[0]),
                                    flags=cv2.INTER_NEAREST)
    ...
    img2_warped = cv2.warpPerspective(img2, np.float32(mat2_estim),
                                      (panorama_kf_estim.shape[1], panorama_kf_estim.shape[0]), flags=cv2.INTER_NEAREST)
    mask_img2 = cv2.warpPerspective(mask, np.float32(mat2_estim),
                                    (panorama_kf_estim.shape[1], panorama_kf_estim.shape[0]), flags=cv2.INTER_NEAREST)
...
def sanity_check(frame_to_relocalize_transf, path_frames, matcher, panorama, mask, matrix_frame_to_relocalize,
...
def main():
    ...
    list_videos = sorted(
        os.listdir(os.path.join(os.getcwd(), '../dataset_MICCAI_2020/dataset')))  # contains list of videos names
    ...
    for name_video in list_videos:
        ...
        tuple_result = sanity_check(frame_to_relocalize_transf, path_frames, matcher, panorama, mask,
