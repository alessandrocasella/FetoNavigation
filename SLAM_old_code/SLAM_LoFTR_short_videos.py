...
def compute_key_frames (list_frames, mask, matcher, initial_matrix, name_video):
    ...
    return list_key_frames, mkpts0_f_int, mkpts1_f_int, keypoints_img0, keypoints_img1, list_homographies
...
# It is the algorithm for the reconstruction of the panorama based on the given matrices
def panorama_reconstruction_optimized(list_images, list_matrices, path_images, name_video, mask, name_experiment):
    path_panorama_image_folder = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/output_panorama_images', name_experiment)
    if not os.path.exists(path_panorama_image_folder):
        #print('creating new panorama image folder')
        os.makedirs(path_panorama_image_folder)
    ...
    if not os.path.exists(path_panorama_image):
        ...
        mask = kornia.image_to_tensor(mask).to(device).unsqueeze(0)
        print(mask.shape)
        ...
        canvas_center = np.array(
            [[np.cos(0), np.sin(0), black_canvas.shape[1] / 2], [-np.sin(0), np.cos(0), black_canvas.shape[0] / 2],
             [0, 0, 1]])
        canvas_center= torch.FloatTensor(canvas_center).to(device).view(1, 3, 3)
        for index, image_name in enumerate(list_images):

            image = np.float32(cv2.imread(os.path.join(os.getcwd(),path_images,image_name),1)) / 255.
            image = cv2.resize(image, (NEW_SHAPE, NEW_SHAPE))

            image = kornia.image_to_tensor(image).to(device).unsqueeze(0)
            image = torch.cat((image, mask), dim=1)
            output = torch.zeros((1, 4, black_canvas.shape[0], black_canvas.shape[1])).to(device)

            if (index == 0):
                img_origin = np.array([[np.cos(0), np.sin(0), NEW_SHAPE/2], [-np.sin(0), np.cos(0), NEW_SHAPE/2], [0, 0, 1]])
                img_origin = torch.FloatTensor(img_origin).to(device).view(1, 3, 3)
                H = torch.matmul(img_origin, canvas_center)


            else:
                H_hat= torch.FloatTensor(list_matrices[index-1]).to(device).view(1, 3, 3)
                H = torch.matmul(H, H_hat)


            image = kornia.geometry.warp_perspective(image, H, (black_canvas.shape[0], black_canvas.shape[0]))
            print(image.shape)
            print(output.shape)
            output = torch.where(image[0, 3, :, :] > 0, image, output)
            outputC = kornia.tensor_to_image(output)
            outputC = np.uint8(outputC * 255.)
            outputC = cv2.cvtColor(outputC, cv2.COLOR_RGBA2RGB)
            out.write(outputC)

        out.release()
        path_panorama = os.path.join(os.getcwd(), '../dataset_MICCAI_2020_files/output_panorama_images', name_experiment, 'panorama_' + name_video + '.png')
        #print(path_panorama)
        cv2.imwrite(path_panorama, outputC)
        cv2.destroyAllWindows()
...
def sanity_check(frame_to_relocalize_transf, list_key_frames_names, path_frames, matcher, panorama, mask, matrices_key_frames, matrix_frame_to_relocalize, string_name_panorama):
    ...
    # print('len of matrices key frames: ', len(matrices_key_frames))
    # print('key frame min dist',list_key_frames_names[tuple_result[0]])
    # print('homography matrix of the key frame:')
    # print(matrices_key_frames[tuple_result[0]])
    ...
    #cv2.imwrite(os.path.join(os.getcwd(), 'key_frame_warped.png'), key_frame_warped)
    ...
    #cv2.imwrite(os.path.join(os.getcwd(), 'mask_key_frame.png'), mask_key_frame)
    ...
    #cv2.imwrite(os.path.join(os.getcwd(), 'panorama_with_key_frame.png'), panorama_with_key_frame)
   ...
   #cv2.imwrite(os.path.join(os.getcwd(), 'panorama_with_key_frame_dilatation.png'), panorama_with_key_frame)


    #print(matrix_frame_to_relocalize)
    ...
    #cv2.imwrite(os.path.join(os.getcwd(), 'image_to_check_warped.png'), image_to_check_warped)
    ...
    #cv2.imwrite(os.path.join(os.getcwd(), 'mask_image_to_check.png'), mask_image_to_check)
    ...
    #cv2.imwrite(os.path.join(os.getcwd(), 'panorama_with_key_frame_image_check_warped.png'), panorama_with_key_frame)
    ...
    dilatation = cv2.merge([b, g, r])   # dilatation with red color
    np.copyto(panorama_with_key_frame, dilatation, where=dilatation.astype(bool))
    #cv2.imwrite(os.path.join(os.getcwd(), 'panorama_final.png'), panorama_with_key_frame)
    ...
    tuple_to_return = (tuple_result[0], list_key_frames_names[tuple_result[0]])
...
NUM_POINTS_THRESHOLD = 60
device = torch.device('cuda:0' if torch.cuda.is_available() else "cpu")
name_experiment = 'SLAM_LoFTR_short_videos_75_60' #string containing the name used to identify the experiment
...
for name_video in list_videos:
    ...
    list_key_frames_names, mkpts0_f_int, mkpts1_f_int, keypoints_img0, keypoints_img1, list_homografies = compute_key_frames(list_frames, mask, matcher, initial_matrix, name_video)

    matrices_key_frames = []

    count=0
    for i in range(len(list_frames)):
        if list_frames[i] in list_key_frames_names:
            count +=1
            matrix = initial_matrix
            for count in range(i):
                matrix = matrix @ list_homografies[count]
            matrices_key_frames.append(matrix)


    panorama = panorama_reconstruction(list_frames, list_homografies, path_frames, name_video, mask, name_experiment)
    # print('len list_key_frames_names', len(list_key_frames_names))
    # print('len matrices_key_frames', len(matrices_key_frames))
    # print('count', count)
    #print(matrices_key_frames)


    num_frame_to_relocalize = 51
    name_frame_to_relocalize = list_frames[num_frame_to_relocalize]
    #print(name_frame_to_relocalize)
