...
path_frames = os.path.join(os.getcwd(), '../../../final_dataset/anon024_CLIP00')
list_frames_names = sorted(os.listdir(os.path.join(path_frames, 'predicted_mask')))
errorThresh = 0.0001

path_matrices = os.path.join(os.getcwd(), '../../../final_dataset/anon024_CLIP00/output_sp_RANSAC')
...
for i in range(len(list_frames_names)):
	...
	if not os.path.exists(os.path.join(path_matrices, name_matrix + '.txt')):
